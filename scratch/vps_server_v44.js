...
function loadKeys() {
    try {
        ...
        console.log('[v44] Keys loaded');
    } catch (e) { console.error('[v44] Key load error:', e.message); }
}
loadKeys();
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (!modifiedAwc) { console.log(`[v44] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v44] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    const headerLen = entryCount * 16 + namesLength;
    ...
    console.log(`[v44] Replaced audio in ${count} AWC files (fully decrypted, re-encrypted as AES)`);
    return decrypted;
}
...
async function handleInject(req, res, ver) {
    ...
    try {
        ...
        console.log(`[v44] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error(`[v44] Error:`, e); res.status(500).send(e.message); }
}

app.post('/api/Sound/inject', upload.any(), (req, res) => handleInject(req, res, 'v44'));
app.post('/api/SoundInjector/inject', upload.any(), (req, res) => handleInject(req, res, 'v44'));
...
app.listen(port, '0.0.0.0', () => console.log(`[v44] API Ready on port ${port}`));
