...
function loadKeys() {
    try {
        ...
        console.log('[v42] Keys loaded');
    } catch (e) { console.error('[v42] Key load error:', e.message); }
}
loadKeys();
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (!modifiedAwc) { console.log(`[v42] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v42] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    const headerLen = entryCount * 16 + namesLength;
    ...
    console.log(`[v42] Replaced audio in ${count} AWC files (fully decrypted, re-encrypted as AES)`);
    return decrypted;
}

async function handleInject(req, res, ver) {
    const audioFile = req.files?.['audio']?.[0] || req.files?.['audioFile']?.[0] || req.files?.['pcmFile']?.[0];
    const rpfFile = req.files?.['rpf']?.[0] || req.files?.['rpfFile']?.[0];
    const useTemplate = req.body.useTemplate === 'true';

    if (!audioFile) return res.status(400).send('Missing audio file (expected audio, audioFile or pcmFile)');
    if (!rpfFile && !useTemplate) return res.status(400).send('Missing RPF file (expected rpf or rpfFile)');
    ...
    try {
        ...
        console.log(`[v42] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error(`[v42] Error:`, e); res.status(500).send(e.message); }
}

app.post('/api/Sound/inject', upload.any(), (req, res) => handleInject(req, res, 'v42'));
app.post('/api/SoundInjector/inject', upload.any(), (req, res) => handleInject(req, res, 'v42'));
...
app.listen(port, '0.0.0.0', () => console.log(`[v42] API Ready on port ${port}`));
