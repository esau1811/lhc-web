...
function loadKeys() {
    try {
        ...
        console.log('[v67] Keys loaded');
    } catch (e) { console.error('[v67] Key load error:', e.message); }
}
loadKeys();
...
async function handle(req, res) {
    ...
    try {
        ...
        console.log('[v67] Applying ArchiveFix to ' + rpf.originalname);
        try {
            ...
            console.log('[v67] ArchiveFix Output: ' + out.toString());
            
            // Verify if file changed
            const stats = fs.statSync(rpfPath);
            console.log('[v67] RPF Size after fix: ' + stats.size);

            console.log('[v67] ArchiveFix applied successfully.');
        } catch (afErr) {
            console.error('[v67] ArchiveFix failed:', afErr.message);
...
app.listen(port, '0.0.0.0', () => console.log(`[v67] API Ready on port ${port}`));
