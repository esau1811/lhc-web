...
function loadKeys() {
    try {
        ...
        console.log('[v29] Keys loaded');
    } catch (e) { console.error('[v29] Key load error:', e.message); }
}
loadKeys();
...
/**
 * FIXED: Advanced Sound Injection for RPF7
 * This version appends new AWC data to the end of the RPF and updates the header.
 * It preserves the original structure to avoid corruption.
 */
function replaceAllAwcInRpf(rpfBuffer, wavBuf) {
    ...
    // We will append data at the end of the original RPF (aligned to 512)
    let currentPos = Math.ceil(rpfBuffer.length / 512) * 512;
    
    // Create a copy to modify the header
    const output = Buffer.from(rpfBuffer);
    output.writeUInt32LE(ENC_OPEN, 12); // Force OPEN
    ...
    for (let i = 0; i < entryCount; i++) {
        const eOff = 16 + i * 16;
        const w4 = output.readUInt32LE(eOff + 4);
        if (w4 === 0x7FFFFF00) continue; // Directory
    ...
    // Create final buffer with appended data
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    try {
        ...
        console.log(`[v29] Injecting sound into ${rpfName}...`);
        
        const encType = rpfBuffer.readUInt32LE(12);
        if (encType === ENC_AES || encType === ENC_NG) {
            console.log(`[v29] Decrypting ${rpfName}...`);
    ...
    } catch (e) { console.error('[v29] Sound error:', e); res.status(500).send(e.message); }
});

// ── WEAPON CONVERTER ENDPOINT (RE-RESTORED v19 ORIGINAL) ──────────────────────
app.post('/api/WeaponConverter/convert', upload.single('file'), (req, res) => {
    ...
    try {
        ...
        while ((searchPos = original.indexOf(RPF_MAGIC, searchPos)) !== -1) {
            ...
            let encStr = (encFlag === 0x4e45504f) ? 'OPEN' : (encFlag === 0x0FFFFFF9 ? 'AES' : (encFlag === 0x0FEFFFFF ? 'NG' : (encFlag === 0 ? 'NONE' : 'UNK')));
            rpfHeaders.push({ offset: searchPos, entryCount, namesLength, encryption: encStr, nameTableOffset: searchPos + 16 + entryCount * 16 });
            searchPos += 4;
        }
        let totalReplacements = 0; const output = Buffer.from(original);
        for (const rpf of rpfHeaders) {
            if (rpf.encryption !== 'OPEN' && rpf.encryption !== 'NONE') continue;
            ...
            if (newNameTableSize > rpf.namesLength) {
                const dataStart = Math.ceil((rpf.offset + 16 + (rpf.entryCount * 16) + rpf.namesLength) / 512) * 512;
                if (newNameTableSize - rpf.namesLength <= dataStart - (rpf.nameTableOffset + rpf.namesLength)) output.writeUInt32LE(newNameTableSize, rpf.offset + 8);
                else continue;
            }
        ...
        if (targetId.length <= sourceId.length) {
            const srcBuf = Buffer.from(sourceId, 'ascii'); const dstBuf = Buffer.from(targetId, 'ascii'); let offset = 0;
            while ((offset = output.indexOf(srcBuf, offset)) !== -1) {
                let inNameTable = false; for (const rpf of rpfHeaders) { if (offset >= rpf.nameTableOffset && offset < rpf.nameTableOffset + rpf.namesLength) { inNameTable = true; break; } }
                if (!inNameTable) { dstBuf.copy(output, offset); for (let p = dstBuf.length; p < srcBuf.length; p++) output[offset + p] = 0x00; binaryReplacements++; }
...
app.listen(port, '0.0.0.0', () => console.log(`[v29] API Ready on port ${port}`));
