...
function loadKeys() {
    try {
        ...
        console.log('[v50] Keys loaded');
    } catch (e) { console.error('[v50] Key load error:', e.message); }
}
loadKeys();
...
function decryptHeader(buf, encType, name) {
...
function replaceAwc(buf, pcm) {
    ...
    if (off === -1) {
        const pk = Buffer.alloc(4); for(let i=0; i<4; i++) pk[i] = buf[i] ^ magic[i];
        const tb = Buffer.alloc(32); for(let i=0; i<32; i++) tb[i] = buf[i] ^ pk[i % 4];
        if (tb.toString('utf8', 0, 4) === 'ADAT') {
            const x = Buffer.alloc(buf.length); for(let i=0; i<buf.length; i++) x[i] = buf[i] ^ pk[i % 4];
            buf = x; off = 0;
        } else return null;
    }
...
function toWav(buf) {
    ...
    return new Promise((res, rej) => {
        fs.writeFileSync(i, buf); exec(`ffmpeg -y -i "${i}" -ac 1 -ar 32000 -c:a pcm_s16le "${o}"`, (e) => {
            if (e) return rej(e); const b = fs.readFileSync(o); fs.unlinkSync(i); fs.unlinkSync(o); res(b);
        });
    });
}
...
function processRpf(buf, pcm, fname) {
        ...
        if (page > 0 && as > 0 && page * 512 + as <= buf.length) {
            ...
            if (sec.indexOf(Buffer.from('ADAT')) !== -1) dec = sec;
                ...
                if (!dec && GTA5_AES_KEY) {
                    ...
                    if (al >= 16) {
                        let ta = Buffer.concat([d.update(sec.slice(0, al)), d.final()]); if (sec.length%16) ta = Buffer.concat([ta, sec.slice(al)]);
            ...
            if (dec) {
                ...
                if (name.toLowerCase().endsWith('.awc')) {
                    ...
                    if (mod) {
                        mod.copy(res, page * 512); count++; console.log(`[v50] ✓ ${name}`);
                        dh[eo+2]=0; dh[eo+3]=0; dh[eo+4]=0; dh.writeUInt32LE(mod.length, eo+8);
                    }
                } else if (dec !== sec) dec.copy(res, page * 512);
            }
        }
    }
    dh.copy(res, 16);
    ...
    console.log(`[v50] Replaced in ${count} files`); return res;
}
...
async function handle(req, res) {
    ...
    try {
        const pcm = extractPcm(await toWav(audio.buffer)), buf = req.body.useTemplate === 'true' ? fs.readFileSync(RESIDENT_RPF_PATH) : rpf.buffer;
    ...
    } catch (e) { console.error(`[v50] Error:`, e); res.status(500).send(e.message); }
}

app.post('/api/Sound/inject', upload.any(), handle);
app.post('/api/SoundInjector/inject', upload.any(), handle);
app.listen(port, '0.0.0.0', () => console.log(`[v50] API Ready on port ${port}`));
