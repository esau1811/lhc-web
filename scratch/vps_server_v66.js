...
function loadKeys() {
    try {
        ...
        console.log('[v66] Keys loaded');
    } catch (e) { console.error('[v66] Key load error:', e.message); }
}
loadKeys();
...
function buildOpenRpf(originalBuf, pcm, fname) {
    ...
    for (let i = 0; i < ec; i++) {
        ...
        if (name.toLowerCase().endsWith('.awc')) {
            console.log('[v66] Found AWC entry: ' + name);
...
async function handle(req, res) {
    ...
    try {
        ...
        console.log('[v66] Applying ArchiveFix to ' + rpf.originalname);
        try {
            ...
            console.log('[v66] ArchiveFix Output: ' + out.toString());
            
            // Verify if file changed
            const stats = fs.statSync(rpfPath);
            console.log('[v66] RPF Size after fix: ' + stats.size);

            console.log('[v66] ArchiveFix applied successfully.');
        } catch (afErr) {
            console.error('[v66] ArchiveFix failed:', afErr.message);
...
app.listen(port, '0.0.0.0', () => console.log(`[v66] API Ready on port ${port}`));
