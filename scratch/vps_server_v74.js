...
function loadKeys() {
    try {
        ...
        console.log('[v74] Keys loaded');
    } catch (e) { console.error('[v74] Key load error:', e.message); }
}
loadKeys();
...
async function handle(req, res) {
    ...
    try {
        ...
        console.log('[v74] Applying ArchiveFix to ' + rpf.originalname);
        try {
            ...
            console.log('[v74] ArchiveFix Output: ' + out.toString());
            
            // Verify if file changed
            const stats = fs.statSync(rpfPath);
            console.log('[v74] RPF Size after fix: ' + stats.size);

            console.log('[v74] ArchiveFix applied successfully.');
        } catch (afErr) {
            console.error('[v74] ArchiveFix failed:', afErr.message);
...
app.listen(port, '0.0.0.0', () => console.log(`[v74] API Ready on port ${port}`));
