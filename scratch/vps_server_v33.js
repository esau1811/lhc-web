...
function loadKeys() {
    try {
        ...
        console.log('[v33] Keys loaded');
    } catch (e) { console.error('[v33] Key load error:', e.message); }
}
loadKeys();
...
function replaceAudioInAwc(awcBuf, pcmData) {
    ...
    if (!streams) { console.log('[v33] AWC parse failed'); return null; }
    ...
    for (const stream of streams) {
        ...
        replaced = true;
        console.log(`[v33] Replaced ${toWrite.length} bytes of audio data in-place (max ${maxBytes})`);
    }
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (!modifiedAwc) { console.log(`[v33] Skipping ${name}`); continue; }
        // Write modified AWC back as UNENCRYPTED (for mods folder)
        modifiedAwc.copy(output, awcOffset);
        count++;
        console.log(`[v33] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    // Write decrypted header and set encryption to OPEN
    decHeader.copy(output, 16);
    output.writeUInt32LE(ENC_OPEN, 12);
    console.log(`[v33] Replaced audio in ${count} AWC files`);
    return output;
}
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    try {
        ...
        console.log(`[v33] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error('[v33] Error:', e); res.status(500).send(e.message); }
});
...
app.listen(port, '0.0.0.0', () => console.log(`[v33] API Ready on port ${port}`));
