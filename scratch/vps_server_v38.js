...
function loadKeys() {
    try {
        ...
        console.log('[v38] Keys loaded');
    } catch (e) { console.error('[v38] Key load error:', e.message); }
}
loadKeys();
    ...
    if (encType === ENC_NG) {
        for (let i = 0; i < entryCount; i++) {
            ...
            if (page > 0 && size > 0 && page * 512 + size <= rpfBuf.length) {
                ...
                // 1. Scan for ADAT in plain sector
                if (sector.indexOf(Buffer.from('ADAT')) !== -1) {
                    sector.copy(result, page * 512);
                } else {
                    // 2. Try predicted key + scan
                    const kIdx = ((gta5Hash(name.toLowerCase()) + size + 61) >>> 0) % 101;
                    let decryptedFile = ngDecrypt(sector, GTA5_NG_KEYS[kIdx]);
                    if (decryptedFile.indexOf(Buffer.from('ADAT')) !== -1) {
                        decryptedFile.copy(result, page * 512);
                    } else {
                        // 3. Brute force keys + scan first 64 bytes
                        let found = false;
                        const first64 = sector.slice(0, Math.min(size, 64));
                        for (let k = 0; k < 101; k++) {
                            const d = ngDecrypt(first64, GTA5_NG_KEYS[k]);
                            if (d.indexOf(Buffer.from('ADAT')) !== -1) {
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (!modifiedAwc) { console.log(`[v38] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v38] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    const headerLen = entryCount * 16 + namesLength;
    ...
    console.log(`[v38] Replaced audio in ${count} AWC files (fully decrypted, re-encrypted as AES)`);
    return decrypted;
}
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    try {
        ...
        console.log(`[v38] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error('[v38] Error:', e); res.status(500).send(e.message); }
});
...
app.listen(port, '0.0.0.0', () => console.log(`[v38] API Ready on port ${port}`));
