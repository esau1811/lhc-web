...
async function main() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        console.log(`  Raw: ${headerDec.slice(eOff, eOff+16).toString('hex')}`);
        ...
        const val12_15 = headerDec.readUInt32LE(eOff + 12);
        
        console.log(`  Size24: ${size24}, Page24: ${page24}, val8-11: ${val8_11}, val12-15: ${val12_15}`);
        
        if (name.endsWith('.awc')) {
            const offset = page24 * 512;
            const kIdx = ((gta5Hash(name.toLowerCase()) + size24 + 61) >>> 0) % 101;
            const data = ngDecrypt(rpfBuf.slice(offset, offset + 16), ngKeys[kIdx], tables);
            console.log(`  AWC Magic: ${data.slice(0, 4).toString('hex')} (${data.slice(0, 4).toString('utf8')})`);
        }
    }
}
