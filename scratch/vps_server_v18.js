...
app.get('/ping', (req, res) => res.send('pong v18'));

// ──────────────────────────────────────────────────────────────────────────────
// SOUND INJECT  POST /api/Sound/inject
// Accepts: audio (MP3/WAV) + rpf (weapon RPF) as multipart form-data
// Returns: ZIP with "LHC Sound boost/WEAPONS_PLAYER.rpf" + RESIDENT.rpf
// ──────────────────────────────────────────────────────────────────────────────
app.post('/api/Sound/inject',
    ...
    (req, res) => {
        ...
        try {
            ...
            // User's RPF → WEAPONS_PLAYER.rpf inside the folder
            zip.addFile('LHC Sound boost/WEAPONS_PLAYER.rpf', rpfFile.buffer);

            // Static RESIDENT.rpf from server disk (if uploaded)
            ...
            res.status(500).send('Error al generar el ZIP: ' + e.message);
        }
    }
);
...
app.get('/api/Sound/test', (req, res) => {
    ...
    res.send(`Sound API v1.0 | RESIDENT.rpf: ${hasResident ? 'OK' : 'NOT FOUND (' + RESIDENT_RPF_PATH + ')'}`);
});
    ...
    try {
        ...
        if (totalReplacements === 0) {
            return res.status(400).send(
                `No replacements made. Source "${sourceId}" not found in any RPF name table. Log: ${log.join(' | ')}`
            );
        }
...
app.listen(port, '0.0.0.0', () => {
    console.log(`[v18] Sound+Converter API listening on port ${port}`);
});
