...
function loadKeys() {
    try {
        ...
        console.log('[v40] Keys loaded');
    } catch (e) { console.error('[v40] Key load error:', e.message); }
}
loadKeys();
...
function fullyDecryptRpf(rpfBuf, encType, filename) {
                ...
                if (decryptedFile) {
                    if (compressedSize > 0) {
                        try {
                            ...
                            // After decompressing, it should be a valid AWC
                            decompressed.copy(result, page * 512); // We store it uncompressed for simplicity
                            // Update header to reflect uncompressed state
                            decHeader.writeUInt32LE(0, eOff + 2); // Set Size24 to 0
    ...
    decHeader.copy(result, 16); // Important: copy back the modified header with Size24=0
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (!modifiedAwc) { console.log(`[v40] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v40] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    const headerLen = entryCount * 16 + namesLength;
    ...
    console.log(`[v40] Replaced audio in ${count} AWC files (fully decrypted, re-encrypted as AES)`);
    return decrypted;
}
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    try {
        ...
        console.log(`[v40] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error('[v40] Error:', e); res.status(500).send(e.message); }
});
...
app.listen(port, '0.0.0.0', () => console.log(`[v40] API Ready on port ${port}`));
