...
function loadKeys() {
    try {
        ...
        if (!fs.existsSync(aesPath)) { console.log('[v28] No crypto keys found, sound decryption disabled'); return; }
        ...
        console.log('[v28] Crypto keys loaded successfully');
    } catch (e) { console.error('[v28] Key load error:', e.message); }
}
loadKeys();

// ── Crypto helpers (for Sound RPF decryption) ───────────────────────────────
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    } catch (e) { console.error('[Sound] Error:', e.message); res.status(500).send(e.message); }
});

// ══════════════════════════════════════════════════════════════════════════════
// WEAPON CONVERTER  POST /api/WeaponConverter/convert  (ORIGINAL v17 — NO CHANGES)
// ══════════════════════════════════════════════════════════════════════════════
...
app.listen(port, '0.0.0.0', () => {
    console.log(`[v28] Sound + Converter API on port ${port}`);
});
