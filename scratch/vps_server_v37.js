...
function loadKeys() {
    try {
        ...
        console.log('[v37] Keys loaded');
    } catch (e) { console.error('[v37] Key load error:', e.message); }
}
loadKeys();
...
function replaceAudioInAwc(awcBuf, pcmData) {
    ...
    if (!streams) { console.log('[v37] AWC parse failed'); return null; }
    ...
    for (const stream of streams) {
        ...
        replaced = true;
        console.log(`[v37] Replaced ${toWrite.length} bytes of audio data in-place (max ${maxBytes})`);
    }
        ...
        for (let i = 0; i < entryCount; i++) {
            ...
            if (page > 0 && size > 0 && page * 512 + size <= rpfBuf.length) {
                ...
                if (sector.length >= 4 && sector.readUInt32BE(0) === 0x41444154) {
                    ...
                    let decryptedFile = ngDecrypt(sector, GTA5_NG_KEYS[kIdx]);
                    if (decryptedFile.length >= 4 && decryptedFile.readUInt32BE(0) === 0x41444154) {
                        decryptedFile.copy(result, page * 512);
                    } else {
                        let found = false;
                        for (let k = 0; k < 101; k++) {
                            const d = ngDecrypt(sector.slice(0, 16), GTA5_NG_KEYS[k]);
                            if (d.readUInt32BE(0) === 0x41444154) {
                                ngDecrypt(sector, GTA5_NG_KEYS[k]).copy(result, page * 512);
                                found = true; break;
                            }
                        }
                        if (!found) sector.copy(result, page * 512);
                    }
                }
            }
        }
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (!modifiedAwc) { console.log(`[v37] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v37] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    const headerLen = entryCount * 16 + namesLength;
    ...
    console.log(`[v37] Replaced audio in ${count} AWC files (fully decrypted, re-encrypted as AES)`);
    return decrypted;
}
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    try {
        ...
        console.log(`[v37] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error('[v37] Error:', e); res.status(500).send(e.message); }
});
...
app.listen(port, '0.0.0.0', () => console.log(`[v37] API Ready on port ${port}`));
