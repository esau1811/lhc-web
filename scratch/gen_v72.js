...
const v71 = fs.readFileSync('C:\\Users\\esau2\\.gemini\\antigravity\\scratch\\LHC\\scratch\\vps_server_v71.js', 'utf8');

// IMPROVED buildOpenRpf with ADPCM support
const updatedBuildRpf = `
...
let v72 = v71.replace(/\[v71\]/g, '[v72]');
v72 = v72.replace(/function buildOpenRpf[\s\S]+?\n}/, updatedBuildRpf);
v72 = v72.replace('const pcm = fs.readFileSync(audioPath);', `const adpcmData = await (async () => {
                const outputPath = audioPath + '.adpcm';
                try {
                    require('child_process').execSync(\`ffmpeg -y -i "\${audioPath}" -acodec adpcm_ima_wav -ar 32000 -ac 1 "\${outputPath}"\`);
                    const wav = fs.readFileSync(outputPath);
                    const dataIdx = wav.indexOf('data');
                    if (dataIdx === -1) return wav.slice(44);
                    const dataSize = wav.readUInt32LE(dataIdx + 4);
                    return wav.slice(dataIdx + 8, dataIdx + 8 + dataSize);
                } catch (e) {
                    console.error('Conversion failed', e);
                    return fs.readFileSync(audioPath);
                }
            })();`);
v72 = v72.replace('const patchedRpf = buildOpenRpf(originalRpf, pcm, originalName);', 'const patchedRpf = buildOpenRpf(originalRpf, adpcmData, originalName);');

fs.writeFileSync('C:\\Users\\esau2\\.gemini\\antigravity\\scratch\\LHC\\scratch\\vps_server_v72.js', v72);
console.log('vps_server_v72.js generated.');
