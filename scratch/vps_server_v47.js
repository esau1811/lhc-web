...
function loadKeys() {
    try {
        ...
        console.log('[v47] Keys loaded');
    } catch (e) { console.error('[v47] Key load error:', e.message); }
}
loadKeys();
...
function ngDecryptBlock(block, keyBuf) {
    ...
    const rdA = (d,s,t) => { const r=Buffer.allocUnsafe(16); r.writeUInt32LE((t[0][d[0]]^t[1][d[1]]^t[2][d[2]]^t[3][d[3]]^s[0])>>>0,0); r.writeUInt32LE((t[4][d[4]]^t[5][d[5]]^t[6][d[6]]^t[7][d[7]]^s[1])>>>0,4); r.writeUInt32LE((t[8][d[8]]^t[9][d[9]]^t[11][d[11]]^t[11][d[11]]^s[2])>>>0,8); r.writeUInt32LE((t[12][d[12]]^t[13][d[13]]^t[14][d[14]]^t[15][d[15]]^s[3])>>>0,12); return r; };
...
function decryptRpfHeader(rpfBuf, encType, filename) {
    ...
    if (encType === ENC_AES) {
        ...
        if (test.readUInt16LE(0) === 0 && test.readUInt32LE(4) === 0x7FFFFF00) {
            ...
            console.log(`[v47] Initial NG key failed for ${filename}, brute-forcing...`);
            for (let i = 0; i < 101; i++) {
                const t = ngDecryptBlock(enc.slice(0,16), GTA5_NG_KEYS[i]);
                if (t.readUInt16LE(0) === 0 && t.readUInt32LE(4) === 0x7FFFFF00) {
                    key = GTA5_NG_KEYS[i];
                    console.log(`[v47] Found correct NG key at index ${i}`);
                    break;
                }
            }
        }
...
function replaceAudioInAwc(awcBuf, pcmData, name) {
    ...
    if (adatOff === -1) { 
        // Forensic: deriving XOR key if it's a simple XOR obfuscation
        const magic = Buffer.from('ADAT');
        const possibleKey = Buffer.alloc(4);
        for(let i=0; i<4; i++) possibleKey[i] = awcBuf[i] ^ magic[i];
        
        const testBuf = Buffer.alloc(32);
        for(let i=0; i<32; i++) testBuf[i] = awcBuf[i] ^ possibleKey[i % 4];
        
        if (testBuf.toString('utf8', 0, 4) === 'ADAT') {
            console.log(`[v47] XOR Key Found for ${name}: ${possibleKey.toString('hex')}`);
            const xored = Buffer.alloc(awcBuf.length);
            for(let i=0; i<awcBuf.length; i++) xored[i] = awcBuf[i] ^ possibleKey[i % 4];
            adatOff = 0;
            awcBuf = xored;
        } else {
            console.log(`[v47] AWC parse failed for ${name}. Raw 16: ${awcBuf.slice(0,16).toString('hex')}`); 
            return null;
        }
    }
    ...
    for (const stream of streams) {
        ...
        replaced = true;
        console.log(`[v47] Replaced ${toWrite.length} bytes in ${name} (off ${adatOff})`);
    }
                ...
                if (decryptedFile) {
                    if (compressedSize > 0 && decryptedFile[0] === 0x78) {
                        try {
                            ...
                            decHeader[eOff+2] = 0; decHeader[eOff+3] = 0; decHeader[eOff+4] = 0;
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (!modifiedAwc) { console.log(`[v47] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v47] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    const headerLen = entryCount * 16 + namesLength;
    ...
    console.log(`[v47] Replaced audio in ${count} AWC files`);
    return decrypted;
}

async function handleInject(req, res, ver) {
    ...
    if (!audioFile || (!rpfFile && !useTemplate)) return res.status(400).send('Missing files');
    try {
        ...
        console.log(`[v47] Processing ${rpfName}`);
        const modifiedRpf = replaceAllAwcInRpf(rpfBuffer, pcmData, rpfBuffer.readUInt32LE(12), rpfName);
    ...
    } catch (e) { console.error(`[v47] Error:`, e); res.status(500).send(e.message); }
}

app.post('/api/Sound/inject', upload.any(), (req, res) => handleInject(req, res, 'v47'));
app.post('/api/SoundInjector/inject', upload.any(), (req, res) => handleInject(req, res, 'v47'));

app.listen(port, '0.0.0.0', () => console.log(`[v47] API Ready on port ${port}`));
