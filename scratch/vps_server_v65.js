...
function loadKeys() {
    try {
        ...
        console.log('[v65] Keys loaded');
    } catch (e) { console.error('[v65] Key load error:', e.message); }
}
loadKeys();
...
async function handle(req, res) {
    ...
    try {
        ...
        console.log('[v65] Applying ArchiveFix to ' + rpf.originalname);
        try {
            ...
            console.log('[v65] ArchiveFix Output: ' + out.toString());
            
            // Verify if file changed
            const stats = fs.statSync(rpfPath);
            console.log('[v65] RPF Size after fix: ' + stats.size);

            console.log('[v65] ArchiveFix applied successfully.');
        } catch (afErr) {
            console.error('[v65] ArchiveFix failed:', afErr.message);
...
app.listen(port, '0.0.0.0', () => console.log(`[v65] API Ready on port ${port}`));
