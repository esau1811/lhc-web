...
function loadKeys() {
    try {
        ...
        console.log('[v35] Keys loaded');
    } catch (e) { console.error('[v35] Key load error:', e.message); }
}
loadKeys();
    ...
    if (encType === ENC_NG) {
        for (let i = 0; i < entryCount; i++) {
            ...
            if (page > 0 && size > 0 && page * 512 + size <= rpfBuf.length) {
                ...
                ngDecrypt(rpfBuf.slice(page * 512, page * 512 + size), GTA5_NG_KEYS[kIdx]).copy(result, page * 512);
            }
        }
    }
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (!modifiedAwc) { console.log(`[v35] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v35] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    ...
    console.log(`[v35] Replaced audio in ${count} AWC files (fully decrypted, re-encrypted as AES)`);
    return decrypted;
}
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    try {
        ...
        console.log(`[v35] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error('[v35] Error:', e); res.status(500).send(e.message); }
});
...
app.listen(port, '0.0.0.0', () => console.log(`[v35] API Ready on port ${port}`));
