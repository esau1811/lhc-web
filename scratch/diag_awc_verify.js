// Verify keys 88 and 93 by decrypting more of the AWC and checking structure
const fs = require('fs');
...
// lmg_combat.awc: page=2, us=55496
// ptl_pistol.awc: page=1874, us=64352
const entries = [
...
for (const e of entries) {
    console.log(`\n=== ${e.name} (page=${e.page}) ===`);
    const rawData = rpf.slice(e.page * 512, e.page * 512 + Math.min(e.us, 512));
    
    for (const keyIdx of [88, 93]) {
        const dec = ngDecryptData(rawData, GTA5_NG_KEYS[keyIdx]);
        const flags = dec.readUInt32LE(0);
        const streams = flags & 0xFFF;
        const streaming = (flags >>> 31) !== 0;
        console.log(`Key[${keyIdx}]: flags=0x${flags.toString(16)} streams=${streams} streaming=${streaming}`);
        console.log(`  Hex: ${dec.slice(0, 32).toString('hex')}`);
        
        // If not streaming (weapon sounds), structure is:
        // 0x00: flags (uint32)
        // 0x08: start of stream headers? 
        // Each stream header: 4 bytes hash + 4 bytes data offset + variable info blocks
        if (!streaming && streams > 0 && streams <= 8) {
            // Try to read what looks like a hash + offset at bytes 8
            const hash = dec.readUInt32LE(8);
            const offset = dec.readUInt32LE(12);
            console.log(`  >> Stream[0]: hash=0x${hash.toString(16)} offset=0x${offset.toString(16)}`);
            if (offset > 0 && offset < e.us && offset < 65536) {
                console.log(`  >> VALID OFFSET! Data starts at 0x${offset.toString(16)}`);
            }
        }
    }
}
