...
async function main() {
    ...
    const rpfBuf = fs.readFileSync('arma/WEAPONS_PLAYER.rpf');
    const entryCount = rpfBuf.readUInt32LE(4);
    const namesLength = rpfBuf.readUInt32LE(8);
    const headerLen = entryCount * 16 + namesLength;
    
    const idx = ((gta5Hash('weapons_player.rpf') + rpfBuf.length + 61) >>> 0) % 101;
    let headerDec;
    try { headerDec = ngDecrypt(rpfBuf.slice(16, 16 + headerLen), ngKeys[idx], tables); }
    catch(e) {
         for(let i=0; i<101; i++) {
             const t = ngDecryptBlock(rpfBuf.slice(16, 32), ngKeys[i], tables);
             if(t.readUInt16LE(0)===0 && t.readUInt32LE(4)===0x7FFFFF00) { headerDec = ngDecrypt(rpfBuf.slice(16, 16+headerLen), ngKeys[i], tables); break; }
         }
    }
    ...
    for (let i = 0; i < entryCount; i++) {
        const eOff = i * 16;
        if (headerDec.readUInt32LE(eOff+4) === 0x7FFFFF00) continue;
        ...
        if (name.endsWith('.awc')) {
            const page = headerDec[eOff+5] | (headerDec[eOff+6]<<8) | (headerDec[eOff+7]<<16);
            const size = headerDec.readUInt32LE(eOff + 8);
            const raw = rpfBuf.slice(page * 512, page * 512 + 64);
            console.log(`AWC ${name} (page ${page}, size ${size}):`);
            console.log(`  RAW: ${raw.slice(0, 32).toString('hex')}`);
        }
    }
}
