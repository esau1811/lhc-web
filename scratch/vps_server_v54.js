...
function loadKeys() {
    try {
        ...
        console.log('[v54] Keys loaded');
    } catch (e) { console.error('[v54] Key load error:', e.message); }
}
loadKeys();
...
function processRpf(buf, pcm, fname) {
                ...
                if (page > 0 && as > 0 && page * 512 + as <= buf.length) {
                    ...
                    if (dec) {
                        ...
                        if (name.toLowerCase().endsWith('.awc')) {
                            ...
                            if (mod) {
                                mod.copy(res, page * 512); count++; console.log(`[v54] ✓ INJECTED: ${currentPath}${name}`);
    ...
    // Only scan the root directory correctly (usually the first entries)
    // To avoid duplicates, we scan starting from index 0 up to the first folder boundary
    // But scan(0, ec) is okay if we handle the root entries correctly.
    // Actually, ec is usually the count of entries in the ROOT.
    scan(0, ec, '');
    dh.copy(res, 16);
    if (GTA5_AES_KEY) {
    ...
    console.log(`[v54] Final: Replaced in ${count} files`); return res;
}
...
async function handle(req, res) {
    ...
    } catch (e) { console.error(`[v54] Error:`, e); res.status(500).send(e.message); }
}

app.post('/api/Sound/inject', upload.any(), handle);
app.post('/api/SoundInjector/inject', upload.any(), handle);
app.listen(port, '0.0.0.0', () => console.log(`[v54] API Ready on port ${port}`));
