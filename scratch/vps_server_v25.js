...
function loadKeys() {
    try {
        ...
        console.log('[v25] Keys loaded');
    } catch (e) { console.error('[v25] Key load error:', e.message); }
}
loadKeys();
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    console.log(`[v25] Sound request: ${req.files?.['rpf']?.[0]?.originalname || 'template'}`);
    ...
    try {
        ...
        console.log(`[v25] Sound success: ${rpfName}`);
    } catch (e) { console.error(`[v25] Sound error: ${e.message}`); res.status(500).send(e.message); }
});
...
app.post('/api/WeaponConverter/convert', upload.single('file'), (req, res) => {
    console.log(`[v25] Converter request: ${req.file?.originalname}`);
    ...
    try {
        ...
        while ((searchPos = original.indexOf(RPF_MAGIC, searchPos)) !== -1) {
            ...
            let enc = 'UNKNOWN'; if (ef === 0x4e45504f) enc = 'OPEN'; else if (ef === 0x0FFFFFF9) enc = 'AES'; else if (ef === 0x0FEFFFFF) enc = 'NG'; else if (ef === 0) enc = 'NONE';
        ...
        for (const rpf of rpfHeaders) {
            ...
            while (pos < nameTableEnd) {
                ...
                names.push({ name, offset: start - rpf.nameTableOffset }); pos++;
            }
            if (names.length === 0) continue;
            let hasChanges = false;
            const newNames = names.map(n => {
                let newName = n.name; if (newName.toLowerCase().includes(sourceId)) { newName = newName.replace(new RegExp(sourceId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'), targetId); hasChanges = true; }
                return { ...n, newName };
            });
            if (!hasChanges) continue;
            const newTableSize = newNames.reduce((s, n) => s + n.newName.length + 1, 0);
            if (newTableSize > rpf.namesLength) {
                const dataStart = Math.ceil((rpf.nameTableOffset + rpf.namesLength) / 512) * 512;
                if (newTableSize - rpf.namesLength <= dataStart - (rpf.nameTableOffset + rpf.namesLength)) { output.writeUInt32LE(newTableSize, rpf.offset + 8); }
                else continue;
            }
            let wp = rpf.nameTableOffset; const newOffsets = [];
            for (const n of newNames) {
                newOffsets.push(wp - rpf.nameTableOffset); const nb = Buffer.from(n.newName, 'ascii'); nb.copy(output, wp); wp += nb.length; output[wp++] = 0;
        ...
        if (targetId.length <= sourceId.length) {
            const srcB = Buffer.from(sourceId, 'ascii'); const dstB = Buffer.from(targetId, 'ascii'); let o = 0;
            while ((o = output.indexOf(srcB, o)) !== -1) {
                ...
                if (!inNT) { dstB.copy(output, o); for (let p = dstB.length; p < srcB.length; p++) output[o + p] = 0; totalReplacements++; }
                o += srcB.length;
            }
        }
        ...
        res.send(output);
        console.log(`[v25] Converter success: ${targetId}.rpf`);
    } catch (e) { console.error('[v25] Converter error:', e); res.status(500).send(e.message); }
});

app.listen(port, '0.0.0.0', () => console.log(`[v25] API Ready on port ${port}`));
