...
function loadKeys() {
    try {
        ...
        console.log('[v64] Keys loaded');
    } catch (e) { console.error('[v64] Key load error:', e.message); }
}
loadKeys();
...
function buildOpenRpf(originalBuf, pcm, fname) {
    ...
    for (const entry of entries) {
        ...
        if (isAwc) {
            const awcData = buildMinimalAwc(pcm); // Unencrypted AWC
            ...
            newDh.writeUInt32LE(0, oldEo + 12); // EncryptionType = 0 (false)
    ...
    Buffer.from([0x52, 0x50, 0x46, 0x37]).copy(result, 0); // "RPF7"
    result.writeUInt32LE(ec, 4);
    result.writeUInt32LE(nl, 8);
    result.writeUInt32LE(0, 12); // NONE Encryption to bypass FiveM's "OPEN" check
    
    newDh.copy(result, 16);
    for (const block of dataBlocks) block.data.copy(result, block.page * PAGE_SIZE);
...
async function handle(req, res) {
    ...
    try {
        ...
        console.log('[v64] Applying ArchiveFix to ' + rpf.originalname);
        try {
            ...
            console.log('[v64] ArchiveFix Output: ' + out.toString());
            
            // Verify if file changed
            const stats = fs.statSync(rpfPath);
            console.log('[v64] RPF Size after fix: ' + stats.size);

            console.log('[v64] ArchiveFix applied successfully.');
        } catch (afErr) {
            console.error('[v64] ArchiveFix failed:', afErr.message);
...
app.listen(port, '0.0.0.0', () => console.log(`[v64] API Ready on port ${port}`));
