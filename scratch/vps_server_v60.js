'use strict';
// v60 — Revert to OPEN encryption (like v58)
// AES encryption (v57 and v59) crashes FiveM's fiPackfile parser.
// FiveM fully supports OPEN encryption natively, even for client-side mods in the base game folder.
...
function loadKeys() {
    try {
        ...
        console.log('[v60] Keys loaded');
    } catch (e) { console.error('[v60] Key load error:', e.message); }
}
loadKeys();
...
function buildOpenRpf(originalBuf, pcm, fname) {
    ...
    result.writeUInt32LE(ENC_NONE, 12); // NONE Encryption (bypass FiveM OPEN check)
    ...
    console.log(`[v60] Built OPEN RPF: ${totalSize} bytes`);
    return result;
}
...
app.get('/health', (req, res) => res.json({ version: 'v60', status: 'ok' }));
app.post('/api/Sound/inject', upload.any(), handle);
app.post('/api/SoundInjector/inject', upload.any(), handle);
app.listen(port, '0.0.0.0', () => console.log(`[v60] API Ready on port ${port}`));
