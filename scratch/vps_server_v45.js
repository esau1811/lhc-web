...
function loadKeys() {
    try {
        ...
        console.log('[v46] Keys loaded');
    } catch (e) { console.error('[v46] Key load error:', e.message); }
}
loadKeys();
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        console.log(`[v46] Analyzing ${name}, raw 16: ${awcData.slice(0,16).toString('hex')} (B64: ${awcData.slice(0,32).toString('base64')})`);
        const modifiedAwc = replaceAudioInAwc(awcData, pcmData, name);
        if (!modifiedAwc) { console.log(`[v46] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v46] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    const headerLen = entryCount * 16 + namesLength;
    ...
    console.log(`[v46] Replaced audio in ${count} AWC files (fully decrypted, re-encrypted as AES)`);
    return decrypted;
}
...
async function handleInject(req, res, ver) {
    ...
    for (const f of allFiles) {
        if (f.buffer && f.buffer.length > 16 && f.buffer.slice(0, 4).equals(RPF_MAGIC)) {
            rpfFile = f;
        } else if (f.buffer) {
            audioFile = f;
        }
    ...
    if (!audioFile) return res.status(400).send('No audio file found in request.');
    if (!rpfFile && !useTemplate) return res.status(400).send('No RPF file found in request.');
    ...
    try {
        ...
        // Debug: save user file for analysis
        fs.writeFileSync('/var/www/lhc-node/debug_user.rpf', rpfBuffer);

        const encType = rpfBuffer.readUInt32LE(12);
        console.log(`[v46] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error(`[v46] Error:`, e); res.status(500).send(e.message); }
}

app.post('/api/Sound/inject', upload.any(), (req, res) => handleInject(req, res, 'v46'));
app.post('/api/SoundInjector/inject', upload.any(), (req, res) => handleInject(req, res, 'v46'));
...
app.listen(port, '0.0.0.0', () => console.log(`[v46] API Ready on port ${port}`));
