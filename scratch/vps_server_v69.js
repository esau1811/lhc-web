...
function loadKeys() {
    try {
        ...
        console.log('[v69] Keys loaded');
    } catch (e) { console.error('[v69] Key load error:', e.message); }
}
loadKeys();
...
async function handle(req, res) {
    ...
    try {
        ...
        console.log('[v69] Applying ArchiveFix to ' + rpf.originalname);
        try {
            ...
            console.log('[v69] ArchiveFix Output: ' + out.toString());
            
            // Verify if file changed
            const stats = fs.statSync(rpfPath);
            console.log('[v69] RPF Size after fix: ' + stats.size);

            console.log('[v69] ArchiveFix applied successfully.');
        } catch (afErr) {
            console.error('[v69] ArchiveFix failed:', afErr.message);
...
app.listen(port, '0.0.0.0', () => console.log(`[v69] API Ready on port ${port}`));
