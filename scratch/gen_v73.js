...
function loadKeys() {
    try {
        ...
        console.log('[v73] Keys loaded');
    } catch (e) { console.error('[v73] Key load error:', e.message); }
}
loadKeys();
...
function buildOpenRpf(originalBuf, adpcmData, fname) {
    ...
    for (let i = 0; i < ec; i++) {
        ...
        if (name.toLowerCase().endsWith('.awc')) {
            ...
            let dataOffset = 0;
            try {
                if (origAwc.toString('utf8', 0, 4) === 'AWC ') {
                    const streamCount = origAwc.readUInt32LE(8);
                    const tocOffset = origAwc.readUInt32LE(12);
                    if (streamCount > 0) {
                        dataOffset = origAwc.readUInt32LE(tocOffset + 4); 
                    }
                }
            } catch (e) { }
            
            if (dataOffset === 0 || dataOffset >= origAwc.length) dataOffset = 32; 
            
            console.log('[v73] Patching ' + name + ' at dynamic offset: ' + dataOffset);
...
async function handle(req, res) {
    ...
    try {
        ...
        console.log('[v73] Applying ArchiveFix to ' + rpf.originalname);
        try {
            ...
            console.log('[v73] ArchiveFix Output: ' + out.toString());
            
            // Verify if file changed
            const stats = fs.statSync(rpfPath);
            console.log('[v73] RPF Size after fix: ' + stats.size);

            console.log('[v73] ArchiveFix applied successfully.');
        } catch (afErr) {
            console.error('[v73] ArchiveFix failed:', afErr.message);
...
app.listen(port, '0.0.0.0', () => console.log(`[v73] API Ready on port ${port}`));
