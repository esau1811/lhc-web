...
function loadKeys() {
    try {
        ...
        console.log('[v70] Keys loaded');
    } catch (e) { console.error('[v70] Key load error:', e.message); }
}
loadKeys();
...
async function handle(req, res) {
    ...
    try {
        ...
        console.log('[v70] Applying ArchiveFix to ' + rpf.originalname);
        try {
            ...
            console.log('[v70] ArchiveFix Output: ' + out.toString());
            
            // Verify if file changed
            const stats = fs.statSync(rpfPath);
            console.log('[v70] RPF Size after fix: ' + stats.size);

            console.log('[v70] ArchiveFix applied successfully.');
        } catch (afErr) {
            console.error('[v70] ArchiveFix failed:', afErr.message);
...
app.listen(port, '0.0.0.0', () => console.log(`[v70] API Ready on port ${port}`));
