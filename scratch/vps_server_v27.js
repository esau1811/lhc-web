...
function loadKeys() {
    try {
        ...
        console.log('[v27] Keys loaded');
    } catch (e) { console.error('[v27] Key load error:', e.message); }
}
loadKeys();
    ...
    try {
        ...
        for (const rpf of rpfHeaders) {
            ...
            if (newTableSize > rpf.namesLength) {
                ...
                if (newTableSize - rpf.namesLength <= padding) output.writeUInt32LE(newTableSize, rpf.offset + 8);
                else continue; 
            }
        ...
        res.setHeader('Access-Control-Expose-Headers', 'X-Replacement-Count, Content-Disposition');
        res.send(output);
    } catch (e) { res.status(500).send(e.message); }
});

app.listen(port, '0.0.0.0', () => console.log(`[v27] API Ready`));
