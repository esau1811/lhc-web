...
function loadKeys() {
    try {
        ...
        console.log('[v36] Keys loaded');
    } catch (e) { console.error('[v36] Key load error:', e.message); }
}
loadKeys();
...
function replaceAudioInAwc(awcBuf, pcmData) {
    ...
    if (!streams) { console.log('[v35] AWC parse failed'); return null; }
    ...
    for (const stream of streams) {
        ...
        replaced = true;
        console.log(`[v35] Replaced ${toWrite.length} bytes of audio data in-place (max ${maxBytes})`);
    }
...
function fullyDecryptRpf(rpfBuf, encType, filename) {
    ...
    // Decrypt header
    const enc = rpfBuf.slice(16, 16 + headerLen);
    let decHeader, ngHeaderKey = null;
    if (encType === ENC_AES) {
        const d = crypto.createDecipheriv('aes-256-ecb', GTA5_AES_KEY, null); d.setAutoPadding(false);
        decHeader = Buffer.concat([d.update(enc.slice(0, Math.floor(enc.length/16)*16)), d.final()]);
        if (enc.length % 16) decHeader = Buffer.concat([decHeader, enc.slice(decHeader.length)]);
    } else {
        const idx = ((gta5Hash(filename) + rpfBuf.length + 61) >>> 0) % 101;
        const test = ngDecryptBlock(enc.slice(0,16), GTA5_NG_KEYS[idx]);
        ...
        decHeader = ngDecrypt(enc, ngHeaderKey);
    }
    decHeader.copy(result, 16);
    // Decrypt ALL data sectors (for NG, each file has its own key)
    ...
    if (encType === ENC_NG) {
        for (let i = 0; i < entryCount; i++) {
            ...
            if (page > 0 && size > 0 && page * 512 + size <= rpfBuf.length) {
                const sector = rpfBuf.slice(page * 512, page * 512 + size);
                // Only decrypt if it's not already plaintext (check for 'ADAT' magic)
                if (sector.length >= 4 && sector.readUInt32BE(0) === 0x41444154) {
                    sector.copy(result, page * 512);
                } else {
                    const kIdx = ((gta5Hash(name.toLowerCase()) + size + 61) >>> 0) % 101;
                    ngDecrypt(sector, GTA5_NG_KEYS[kIdx]).copy(result, page * 512);
                }
            }
        }
    }
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    // Step 1: Fully decrypt the RPF
    const { decrypted, decHeader } = fullyDecryptRpf(rpfBuf, encType, filename);
    const entryCount = rpfBuf.readUInt32LE(4);
    const namesLength = rpfBuf.readUInt32LE(8);
    const nameTableStart = entryCount * 16;
    // Step 2: Find and modify AWC audio in-place
    let count = 0;
    for (let i = 0; i < entryCount; i++) {
        const eOff = i * 16;
        if (decHeader.readUInt32LE(eOff + 4) === 0x7FFFFF00) continue;
        ...
        if (!modifiedAwc) { console.log(`[v36] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v36] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    // Step 3: Re-encrypt ONLY the header with AES (data stays plaintext = correct for AES RPFs)
    const headerLen = entryCount * 16 + namesLength;
    ...
    console.log(`[v36] Replaced audio in ${count} AWC files (fully decrypted, re-encrypted as AES)`);
    return decrypted;
}
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    try {
        ...
        console.log(`[v36] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error('[v36] Error:', e); res.status(500).send(e.message); }
});
...
app.listen(port, '0.0.0.0', () => console.log(`[v36] API Ready on port ${port}`));
