...
class Program
{
    ...
    // ── Level 3: streaming.rpf — contains weapon YTD and optional attachments ──
    static byte[] BuildStreamingRpf(List<(string name, byte[] data)> streamFiles)
    {
        var nl = new List<byte> { 0 };
        var nameOffsets = new uint[streamFiles.Count];
        for (int i = 0; i < streamFiles.Count; i++)
        {
            nameOffsets[i] = (uint)nl.Count;
            nl.AddRange(Encoding.ASCII.GetBytes(streamFiles[i].name));
            nl.Add(0);
        }
        while (nl.Count % 16 != 0) nl.Add(0);

        return BuildRpf7(1 + streamFiles.Count, nl.ToArray(),
            streamFiles.ToArray(),
            (bw, secs) =>
            {
                WriteDirEntry(bw, 0, 1, (uint)streamFiles.Count);
                for (int i = 0; i < streamFiles.Count; i++)
                {
                    var data = streamFiles[i].data;
                    ...
                    {
                        pFlag = (uint)data.Length;
                    }
                    WriteResEntry(bw, nameOffsets[i], (uint)data.Length, secs[i], vFlag, pFlag);
                }
            });
    }
    ...
    // ── Texture Patcher via BCnEncoder ─────────────────────────────────────────
    static byte[] PatchYtdTexture(string ytdPath, byte[] ddsBytes, string targetName)
    {
        ...
        if (items == null || items.Length == 0) throw new Exception("Sin texturas en " + ytdPath);

        int idx = -1;
        for (int i = 0; i < items.Length; i++)
            if (items[i] != null && string.Equals(items[i].Name, targetName, StringComparison.OrdinalIgnoreCase))
            { idx = i; break; }
        ...
        if (newTex == null) throw new Exception("DDSIO.GetTexture null");
        newTex.Name = texName;
    ...
    static void Main(string[] args)
    {
        ...
        var streamFiles = new List<(string name, byte[] data)>();
        ...
        if (!string.IsNullOrEmpty(suppName))
        {
            ...
            {
                streamFiles.Add((suppName + ".ydr", File.ReadAllBytes(suppYdrPath)));
                Console.WriteLine($"  YDR base: OK");
            }
            if (File.Exists(suppHiYdr))
            {
                streamFiles.Add((suppName + "_hi.ydr", File.ReadAllBytes(suppHiYdr)));
            ...
            if (File.Exists(suppYtdPath))
            {
                ...
                if (hasDds)
                {
                    suppYtd = PatchYtdTexture(suppYtdPath, File.ReadAllBytes(suppDds), suppName);
                    Console.WriteLine($"  YTD pintado OK");
                }
                else
                {
                    suppYtd = File.ReadAllBytes(suppYtdPath);
                    Console.WriteLine($"  YTD original OK");
                }
                streamFiles.Add((suppName + ".ytd", suppYtd));
            }
        }
