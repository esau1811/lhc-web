...
function loadKeys() {
    try {
        ...
        console.log('[v32] Keys loaded');
    } catch (e) { console.error('[v32] Key load error:', e.message); }
}
loadKeys();
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    } catch (e) { console.error('[v32] Error:', e); res.status(500).send(e.message); }
});
...
app.listen(port, '0.0.0.0', () => console.log(`[v32] API Ready on port ${port}`));
