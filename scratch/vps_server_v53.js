...
function loadKeys() {
    try {
        ...
        console.log('[v53] Keys loaded');
    } catch (e) { console.error('[v53] Key load error:', e.message); }
}
loadKeys();
...
function processRpf(buf, pcm, fname) {
    ...
    function scan(start, end, currentPath) {
        for (let i = start; i < end; i++) {
            ...
            if (type === 0x7FFFFF00) {
                ...
                console.log(`[v53] Folder: ${currentPath}${name}/`);
                ...
                console.log(`[v53] File: ${currentPath}${name} (size: ${as})`);
                if (page > 0 && as > 0 && page * 512 + as <= buf.length) {
                    ...
                    if (dec) {
                        ...
                        if (mod) {
                            mod.copy(res, page * 512); count++; console.log(`[v53] ✓ INJECTED: ${currentPath}${name}`);
    ...
    console.log(`[v53] Final: Replaced in ${count} files`); return res;
}
...
async function handle(req, res) {
    ...
    try {
        ...
        const pcm = fs.readFileSync(o); // No need for extractPcm if we trust ffmpeg output format
    ...
    } catch (e) { console.error(`[v53] Error:`, e); res.status(500).send(e.message); }
}

app.post('/api/Sound/inject', upload.any(), handle);
app.post('/api/SoundInjector/inject', upload.any(), handle);
app.listen(port, '0.0.0.0', () => console.log(`[v53] API Ready on port ${port}`));
