...
function loadKeys() {
    try {
        ...
        console.log('[v51] Keys loaded');
    } catch (e) { console.error('[v51] Key load error:', e.message); }
}
loadKeys();
...
function replaceAwc(buf, pcm) {
    ...
    if (off === -1) {
        // Intensive signature search in first 1024 bytes
...
function processRpf(buf, pcm, fname) {
    ...
    for (let i = 0; i < ec; i++) {
        ...
        console.log(`[v51] File: ${name} (as:${as} us:${us} cs:${cs})`);
        if (page > 0 && as > 0 && page * 512 + as <= buf.length) {
            ...
            if (dec) {
                ...
                if (mod) {
                    mod.copy(res, page * 512); count++; console.log(`[v51] ✓ INJECTED: ${name}`);
    ...
    console.log(`[v51] Final: Replaced in ${count} files`); return res;
}
...
async function handle(req, res) {
    ...
    } catch (e) { console.error(`[v51] Error:`, e); res.status(500).send(e.message); }
}

app.post('/api/Sound/inject', upload.any(), handle);
app.post('/api/SoundInjector/inject', upload.any(), handle);
app.listen(port, '0.0.0.0', () => console.log(`[v51] API Ready on port ${port}`));
