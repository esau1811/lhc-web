...
function loadKeys() {
    try {
        ...
        console.log('[v71] Keys loaded');
    } catch (e) { console.error('[v71] Key load error:', e.message); }
}
loadKeys();
...
async function handle(req, res) {
    ...
    try {
        ...
        console.log('[v71] Applying ArchiveFix to ' + rpf.originalname);
        try {
            ...
            console.log('[v71] ArchiveFix Output: ' + out.toString());
            
            // Verify if file changed
            const stats = fs.statSync(rpfPath);
            console.log('[v71] RPF Size after fix: ' + stats.size);

            console.log('[v71] ArchiveFix applied successfully.');
        } catch (afErr) {
            console.error('[v71] ArchiveFix failed:', afErr.message);
...
app.listen(port, '0.0.0.0', () => console.log(`[v71] API Ready on port ${port}`));
