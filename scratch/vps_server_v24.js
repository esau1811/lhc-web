...
function loadKeys() {
    try {
        ...
        console.log('[v24] Keys loaded');
    } catch (e) { console.error('[v24] Key load error:', e.message); }
}
loadKeys();
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    console.log(`[v24] Injection request: ${req.files?.['rpf']?.[0]?.originalname || 'template'}`);
    ...
    try {
        ...
        console.log(`[v24] Success: ${rpfName}`);
    } catch (e) { console.error(`[v24] Error: ${e.message}`); res.status(500).send(e.message); }
});
...
function replaceAllAwcInRpf(rpfBuffer, wavBuf) {
    ...
    let replacedCount = 0;
    entries.forEach(e => {
        if (!e.isDir && e.name.toLowerCase().endsWith('.awc') && e.data && e.data.length >= 20) {
            const streamId = e.data.readUInt32LE(0x10) & 0x1FFFFFFF;
            e.data = wavToAwc(wavBuf, streamId);
            replacedCount++;
        }
    });
    if (replacedCount === 0) throw new Error('No se encontró ningún archivo .awc para reemplazar.');
    console.log(`[v24] Replaced ${replacedCount} .awc files in RPF`);
...
app.listen(port, '0.0.0.0', () => console.log(`[v24] API Ready on port ${port}`));
