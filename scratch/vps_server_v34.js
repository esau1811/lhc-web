...
function loadKeys() {
    try {
        ...
        console.log('[v34] Keys loaded');
    } catch (e) { console.error('[v34] Key load error:', e.message); }
}
loadKeys();
...
function decryptRpfHeader(rpfBuf, encType, filename) {
    ...
    if (encType === ENC_AES) {
        ...
        if (!key) for (let i = 0; i < 101; i++) { const t = ngDecryptBlock(enc.slice(0,16), GTA5_NG_KEYS[i]); if (t.readUInt16LE(0)===0 && t.readUInt32LE(4)===0x7FFFFF00) { key=GTA5_NG_KEYS[i]; break; } }
...
// ── AWC IN-PLACE PARSER ─────────────────────────────────────────────────────
// Parse the AWC structure to find where audio data bytes live
...
// Replace audio bytes IN-PLACE inside the AWC buffer
function replaceAudioInAwc(awcBuf, pcmData) {
    const streams = parseAwcTags(awcBuf);
    if (!streams) { console.log('[v34] AWC parse failed'); return null; }
    ...
    for (const stream of streams) {
        ...
        if (!dataTag || dataTag.offset + dataTag.size > awcBuf.length) continue;
        // Write PCM data, truncate or pad to fit original size
        const maxBytes = dataTag.size;
        const toWrite = pcmData.slice(0, maxBytes);
        toWrite.copy(modified, dataTag.offset);
        if (toWrite.length < maxBytes) modified.fill(0, dataTag.offset + toWrite.length, dataTag.offset + maxBytes);
        // Update sample count in format tag
        if (fmtTag && fmtTag.offset + 4 <= awcBuf.length) {
            const newSamples = Math.floor(toWrite.length / 2); // 16-bit mono
            modified.writeUInt32LE(newSamples, fmtTag.offset);
        }
        replaced = true;
        console.log(`[v34] Replaced ${toWrite.length} bytes of audio data in-place (max ${maxBytes})`);
    }
...
// ── MAIN IN-PLACE RPF PROCESSOR ─────────────────────────────────────────────
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    const entryCount = rpfBuf.readUInt32LE(4);
    const namesLength = rpfBuf.readUInt32LE(8);
    // Decrypt header only (to read directory)
    const decHeader = decryptRpfHeader(rpfBuf, encType, filename);
    const nameTableStart = entryCount * 16;
    const output = Buffer.from(rpfBuf); // Copy of ORIGINAL encrypted RPF
    // Decrypt data sectors for each AWC, modify in-place, leave rest untouched
    let count = 0;
    for (let i = 0; i < entryCount; i++) {
        ...
        if (page <= 0 || size < 20 || page * 512 + size > rpfBuf.length) continue;
        const awcOffset = page * 512;
        // Decrypt this AWC data sector
        let awcData;
        if (encType === ENC_NG) {
            const kIdx = ((gta5Hash(name.toLowerCase()) + size + 61) >>> 0) % 101;
            awcData = ngDecrypt(rpfBuf.slice(awcOffset, awcOffset + size), GTA5_NG_KEYS[kIdx]);
        } else if (encType === ENC_AES) {
            const d = crypto.createDecipheriv('aes-256-ecb', GTA5_AES_KEY, null); d.setAutoPadding(false);
            const aligned = rpfBuf.slice(awcOffset, awcOffset + Math.floor(size/16)*16);
            awcData = Buffer.concat([d.update(aligned), d.final()]);
            if (size % 16) awcData = Buffer.concat([awcData, rpfBuf.slice(awcOffset + awcData.length, awcOffset + size)]);
        } else {
            awcData = rpfBuf.slice(awcOffset, awcOffset + size);
        }
        // Replace audio in the decrypted AWC
        const modifiedAwc = replaceAudioInAwc(awcData, pcmData);
        if (!modifiedAwc) { console.log(`[v34] Skipping ${name}`); continue; }
        // Write modified AWC back as UNENCRYPTED (for mods folder)
        modifiedAwc.copy(output, awcOffset);
        count++;
        console.log(`[v34] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    // Re-encrypt header with AES so RPF looks like a valid encrypted file
    const cipher = crypto.createCipheriv('aes-256-ecb', GTA5_AES_KEY, null);
    cipher.setAutoPadding(false);
    const alignedLen = Math.floor(decHeader.length / 16) * 16;
    const encHeader = Buffer.concat([cipher.update(decHeader.slice(0, alignedLen)), cipher.final()]);
    encHeader.copy(output, 16);
    if (decHeader.length > alignedLen) decHeader.slice(alignedLen).copy(output, 16 + alignedLen);
    output.writeUInt32LE(ENC_AES, 12);
    console.log(`[v34] Replaced audio in ${count} AWC files (re-encrypted as AES)`);
    return output;
}

// ── SOUND INJECTION ENDPOINT ────────────────────────────────────────────────
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    try {
        ...
        console.log(`[v34] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error('[v34] Error:', e); res.status(500).send(e.message); }
});

// ── WEAPON CONVERTER (UNTOUCHED) ────────────────────────────────────────────
...
app.listen(port, '0.0.0.0', () => console.log(`[v34] API Ready on port ${port}`));
