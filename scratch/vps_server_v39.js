...
function loadKeys() {
    try {
        ...
        console.log('[v39] Keys loaded');
    } catch (e) { console.error('[v39] Key load error:', e.message); }
}
loadKeys();
        ...
        for (let i = 0; i < entryCount; i++) {
            ...
            if (page > 0 && size > 0 && page * 512 + size <= rpfBuf.length) {
                ...
                if (sector.indexOf(Buffer.from('ADAT')) !== -1) {
                    ...
                    if (testNg.indexOf(Buffer.from('ADAT')) !== -1) {
                        decryptedFile = testNg;
                    } else {
                        // Try 3: AES Decryption
...
function replaceAllAwcInRpf(rpfBuf, pcmData, encType, filename) {
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (!modifiedAwc) { console.log(`[v39] Skipping ${name}`); continue; }
        modifiedAwc.copy(decrypted, awcOffset);
        count++;
        console.log(`[v39] ✓ ${name} (sector ${page}, ${size} bytes)`);
    }
    const headerLen = entryCount * 16 + namesLength;
    ...
    console.log(`[v39] Replaced audio in ${count} AWC files (fully decrypted, re-encrypted as AES)`);
    return decrypted;
}
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    try {
        ...
        console.log(`[v39] Processing ${rpfName}, enc=0x${encType.toString(16)}, size=${rpfBuffer.length}, pcm=${pcmData.length} bytes`);
    ...
    } catch (e) { console.error('[v39] Error:', e); res.status(500).send(e.message); }
});
...
app.listen(port, '0.0.0.0', () => console.log(`[v39] API Ready on port ${port}`));
