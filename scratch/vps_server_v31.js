...
function loadKeys() {
    try {
        ...
        console.log('[v31] Keys loaded');
    } catch (e) { console.error('[v31] Key load error:', e.message); }
}
loadKeys();
...
function wavToAwc(wavBuffer, streamId) {
    ...
    awc.writeUInt32LE(32, 12);
    awc.writeUInt32LE((2 << 29) | (streamId & 0x1FFFFFFF), 16);
    const sfxInfoOff = 64; 
    const writeTag = (type, size, offset, writePos) => {
        // FIXED: Force unsigned 32-bit with >>> 0 to avoid "out of range" error
...
function replaceAllAwcInRpf(rpfBuffer, wavBuf) {
    ...
    for (const awc of newAwcs) {
        ...
        console.log(`[v31] Replaced ${awc.name}`);
    }
    return finalBuffer;
}
...
app.post('/api/Sound/inject', upload.fields([{ name: 'audio' }, { name: 'rpf' }]), async (req, res) => {
    ...
    } catch (e) { console.error('[v31] Error:', e); res.status(500).send(e.message); }
});
...
app.listen(port, '0.0.0.0', () => console.log(`[v31] API Ready on port ${port}`));
