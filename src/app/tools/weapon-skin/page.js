...
import {
  ...
  ChevronDown, AlertTriangle, Minus, Plus, Square, ImagePlus
} from 'lucide-react';
...
export default function WeaponSkinPage() {
  ...
  const [hasTexture, setHasTexture] = useState(false); // muestra placeholder si no hay textura

  const CANVAS_W = 1024;
  const CANVAS_H = 512;

  // Canvas inicia TRANSPARENTE — la cuadrícula viene del CSS
  useEffect(() => {
  ...
  const fill = (x, y, fillColor) => {
    ...
    const match = (i) => Math.abs(data[i]-tR)<40 && Math.abs(data[i+1]-tG)<40 && Math.abs(data[i+2]-tB)<40 && Math.abs(data[i+3]-tA)<40;
    while (stack.length) {
  ...
  const placeText = () => {
    ...
    setTextInput(''); setShowTextBox(false); saveHistory();
  };
  ...
  const gridBg = {
    ...
    `,
    backgroundSize: '64px 64px, 64px 64px, 512px 256px, 512px 256px',
  };
      ...
      <main className="max-w-[1400px] mx-auto px-4 pt-28 pb-16">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
          ...
          <div className="relative">
            <button onClick={() => setWeaponOpen(o => !o)}
            ...
            {weaponOpen && (
              <div className="absolute top-full mt-2 right-0 bg-[#111] border border-white/10 rounded-xl overflow-hidden z-50 min-w-[260px] shadow-2xl">
                {WEAPONS.map(w => (
                  <button key={w.id} onClick={() => { setWeapon(w); setWeaponOpen(false); }}
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1">
            <div className="rounded-2xl overflow-hidden border border-white/10 relative shadow-2xl" style={gridBg}>
              {/* Placeholder — solo cuando no hay textura importada */}
              {!hasTexture && (
                <div className="absolute inset-0 flex flex-col items-center justify-center z-10 pointer-events-none select-none">
                  <ImagePlus size={48} className="text-white/10 mb-4" />
                  <p className="text-white/20 text-sm font-black uppercase tracking-widest mb-1">Sin textura</p>
                  <p className="text-white/10 text-xs">Importa el PNG de CodeWalker para empezar</p>
                  <p className="text-white/8 text-[10px] mt-1">O pinta directamente sobre el fondo</p>
                </div>
              )}
              <canvas
                ref={canvasRef}
              ...
              {showTextBox && (
                <div className="absolute z-30"
                  style={{ left:`${(textPos.x/CANVAS_W)*100}%`, top:`${(textPos.y/CANVAS_H)*100}%`, transform:'translate(-50%,-50%)' }}>
                  <input autoFocus value={textInput}
                    onChange={e => setTextInput(e.target.value)}
                    onKeyDown={e => { if(e.key==='Enter') placeText(); if(e.key==='Escape') setShowTextBox(false); }}
                    placeholder="Escribe aquí..."
                    className="bg-black/90 border border-red-500/60 text-white px-3 py-2 rounded-lg text-sm outline-none backdrop-blur-sm"
          ...
          <div className="lg:w-64 flex flex-col gap-3">
            ...
            {(tool==='brush'||tool==='eraser') && (
              <div className="bg-white/3 border border-white/8 rounded-2xl p-4">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={()=>setBrushSize(s=>Math.max(1,s-2))} className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center hover:bg-white/10"><Minus size={12}/></button>
                  <div className="flex-1 text-center">
                    <div className="text-xl font-black">{tool==='eraser'?brushSize*2:brushSize}</div>
                    <div className="text-[8px] text-zinc-600">px</div>
                  </div>
                  <button onClick={()=>setBrushSize(s=>Math.min(60,s+2))} className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center hover:bg-white/10"><Plus size={12}/></button>
                </div>
                <div className="mt-3 flex justify-center">
                  <div className="rounded-full" style={{ width:Math.min(48,(tool==='eraser'?brushSize*2:brushSize)*1.5), height:Math.min(48,(tool==='eraser'?brushSize*2:brushSize)*1.5), backgroundColor:tool==='eraser'?'#444':color }} />
                </div>
              </div>
            )}
            ...
            {tool==='text' && (
              <div className="bg-white/3 border border-white/8 rounded-2xl p-4">
                <div className="text-[9px] text-zinc-500 font-black uppercase tracking-widest mb-3">Tamaño texto</div>
                <div className="flex items-center gap-2 mb-2">
                  <button onClick={()=>setFontSize(s=>Math.max(10,s-4))} className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center hover:bg-white/10"><Minus size={12}/></button>
                  <span className="flex-1 text-center text-sm font-black">{fontSize}px</span>
                  <button onClick={()=>setFontSize(s=>Math.min(120,s+4))} className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center hover:bg-white/10"><Plus size={12}/></button>
                </div>
                <p className="text-[9px] text-zinc-500">Haz clic en el canvas para colocar.</p>
              </div>
            )}
              ...
              <div className="grid grid-cols-5 gap-1">
                {STICKERS.map(s => (
                  <button key={s} onClick={()=>placeSticker(s)}
                    ...
                    title={`Colocar ${s} en el centro`}>{s}</button>
                ))}
              </div>
              <p className="text-[9px] text-zinc-600 mt-2">Se coloca en el centro del canvas.</p>
            </div>
            ...
            <div className="bg-white/3 border border-white/8 rounded-2xl p-4 flex flex-col gap-2">
              ...
              <label className="flex items-center gap-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 rounded-xl px-3 py-3 cursor-pointer transition-all text-xs font-bold text-red-400">
                <Upload size={14} />
                <span>📁 Importar textura PNG</span>
                <input ref={importRef} type="file" accept=".png,.jpg,.jpeg" className="hidden" onChange={importTexture} />
              </label>
              <button onClick={exportTexture}
            ...
            <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-4">
              ...
              <ol className="text-[10px] text-zinc-400 space-y-1.5 list-decimal list-inside leading-relaxed">
                <li>En CodeWalker → RPF Explorer → busca el <b className="text-white">.ytd</b> del arma</li>
                <li>Clic derecho → <b className="text-white">Export texture as PNG</b></li>
                <li>Haz clic en <b className="text-red-400">Importar textura PNG</b> aquí arriba</li>
                <li>Pinta encima de la textura</li>
                <li>Exporta el resultado y reimportalo en CodeWalker</li>
              </ol>
            </div>
          </div>
        </div>
