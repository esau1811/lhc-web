...
export async function POST(request) {
  try {
    ...
    // Copy weapon base YTD
    const weaponYtdSrc = path.join(ASSETS_DIR, `${weaponName}.ytd`);
    if (fs.existsSync(weaponYtdSrc)) {
    ...
    try { fs.rmSync(cleanAssetsDir, { recursive: true, force: true }); } catch {}

    // ── Wrap in ZIP (solo el RPF, como antes) ────────────────────────────
    const zip = new JSZip();
