...
export async function GET(request, { params }) {
  try {
    ...
    const matches = player.team_id ? db.prepare(`
      SELECT m.*,
...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    vals.push(id);
    db.prepare(`UPDATE players SET ${fields.join(', ')} WHERE id = ? OR discord_id = ?`).run(...vals, id);
    const player = db.prepare(`SELECT * FROM players WHERE id = ? OR discord_id = ?`).get(id, id);
